    ...
    if want_test_ft
      ...
      if want_parallel
        telist_raw = cEn_calcTransferEntropyFT_MT( ...
          thisdata, [ dstidx srcidx ], te_laglist, histbins );

        telist_ext = cEn_calcTransferEntropyFT_MT( ...
          thisdata, [ dstidx srcidx ], te_laglist, histbins, struct() );
      else
      ...
      if want_parallel
        telist_raw = cEn_calcTransferEntropy_MT( ...
          { dstseries, srcseries }, te_laglist, histbins );

        telist_ext = cEn_calcTransferEntropy_MT( ...
          { dstseries, srcseries }, te_laglist, histbins, struct() );
      else
    ...
    if want_test_ft
      ...
      if want_parallel
        thistelist = cEn_calcTransferEntropy_MT( ...
          { dstseries, src1series, src2series }, te_laglist, histbins );
        telist1_raw = thistelist(1,:);
        telist2_raw = thistelist(2,:);
